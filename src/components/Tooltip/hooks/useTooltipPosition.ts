...
const useTooltipPosition = ({ container, triggerRef, tooltipRef, position, triPosition }: UseTooltipPositionProps) => {
  const changeTooltipPosition = useCallback(() => {
    ...
    switch (triPosition) {
      case "left":
        tooltipElement.style.left = `${
          windowScrollX + triggerBoundingRect.left + triggerElement.offsetWidth / 2 - 22
        }px`;
        break;
      case "right":
        tooltipElement.style.left = `${
      ...
      case "center":
        tooltipElement.style.left = `${
          windowScrollX + triggerBoundingRect.left + triggerElement.offsetWidth / 2 - tooltipElement.offsetWidth / 2
        }px`;
        break;
    }
    ...
    switch (position) {
      case "top":
        tooltipElement.style.top = `${windowScrollY + triggerBoundingRect.bottom + gap}px`;
        break;
      case "bottom":
        tooltipElement.style.top = `${
          windowScrollY + triggerBoundingRect.top - tooltipElement.offsetHeight - 6 - gap
        }px`;
        break;
      case "left":
        tooltipElement.style.top = `${
          windowScrollY + triggerBoundingRect.top + triggerElement.offsetHeight / 2 - tooltipElement.offsetHeight / 2
        }px`;
        tooltipElement.style.left = `${windowScrollX + triggerBoundingRect.right + gap}px`;
        break;
      case "right":
        tooltipElement.style.top = `${
          windowScrollY + triggerBoundingRect.top + triggerElement.offsetHeight / 2 - tooltipElement.offsetHeight / 2
        ...
        tooltipElement.style.left = `${
          windowScrollX + triggerBoundingRect.left - tooltipElement.offsetWidth - 6 - gap
        }px`;
        break;
    }
    ...
    if (tooltipBoundingRect.width >= windowClientWidth) {
      ...
      tooltipElement.style.transform = `translate(0, ${position === "top" ? "-100%" : "0"})`;
      ...
      tooltipElement.style.left = `${windowScrollWidth - (tooltipBoundingRect.width + gap)}px`;
      tooltipElement.style.transform = `translate(0, ${position === "top" ? "-100%" : "0"})`;
    }
